...
function Datepicker() {
	...
	this.dpDiv = $('<div id="' + this._mainDivId + '" style="display:none" class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all ui-helper-hidden"></div>');
}
...
$.extend(Datepicker.prototype, {
	...
	_generateHTML: function(inst) {
		...
		if (maxDate) {
			...
			maxDraw = (this._compareDate(maxDraw, '<', minDate) ? minDate : maxDraw);
		...
		var prev = (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
			'<a style="direction:ltr" class="ui-datepicker-prev ui-corner-all" onclick="DP_jQuery.datepicker._adjustDate(\'#' + inst.id + '\', -' + stepMonths + ', \'M\');"' +
		...
		var next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
			'<a style="direction:ltr" class="ui-datepicker-next ui-corner-all" onclick="DP_jQuery.datepicker._adjustDate(\'#' + inst.id + '\', +' + stepMonths + ', \'M\');"' +
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						var unselectable = otherMonth || !daySettings[0] ||
							(this._compareDate(printDate, '<', minDate) || this._compareDate(printDate, '>', maxDate));
						tbody += '<td class="' +
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		minDate = (inst.rangeStart && this._compareDate(selectedDate, '<', minDate) ? selectedDate : minDate);
	...
	_isInRange: function(inst, date) {
		...
		newMinDate = (this._compareDate(inst.rangeStart, '<', newMinDate) ? inst.rangeStart : newMinDate);
		var minDate = newMinDate || this._getMinMaxDate(inst, 'min');
		var maxDate = this._getMinMaxDate(inst, 'max');
		return (!this._compareDate(date, '<', minDate) && !this._compareDate(date, '>', maxDate));
	},
